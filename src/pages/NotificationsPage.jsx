...
const PRIORITY_CONFIG = {
	urgent: { badge: "bg-red-50 text-red-600 border-red-100", label: "Urgent", pulse: true },
	high: { badge: "bg-orange-50 text-orange-600 border-orange-100", label: "High", pulse: false },
	normal: { badge: "bg-gray-50 text-gray-500 border-gray-100", label: "Normal", pulse: false },
	low: { badge: "bg-gray-50 text-gray-400 border-gray-100", label: "Low", pulse: false },
};
		...
		const init = async () => {
			try {
				...
				channel = subscribeToNotifications((newNotification) => {
					if (newNotification.target === "all" || newNotification.target === authUser.email) {
					...
					<div className="bg-white rounded-3xl p-6 md:p-8 shadow-lg flex flex-col gap-6 w-full">
									...
									>
										...
										{tab.count > 0 && (
											<span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
												isActive ? "bg-white/25 text-white" : "bg-gray-100 text-[#8A7060]"
											}`}>
												{tab.count}
											</span>
										)}
						...
						{loading ? (
							<div className="space-y-4 py-4">
								{[...Array(3)].map((_, i) => (
									<div key={i} className="h-20 bg-gray-50 border border-gray-100 rounded-2xl animate-pulse" />
								))}
							</div>
							...
							<div className="space-y-3">
								{filteredNotifications.map((noti) => {
									const typeCfg = NOTIFICATION_TYPES[noti.type] || NOTIFICATION_TYPES.broadcast;
									const priCfg = PRIORITY_CONFIG[noti.priority] || PRIORITY_CONFIG.normal;
									...
									return (
										...
										>
											<div className="p-4 md:p-5 flex gap-4 items-start">
												...
												<div className={`flex-shrink-0 w-11 h-11 rounded-xl ${typeCfg.bg} flex items-center justify-center border ${typeCfg.border} transition-transform duration-300 group-hover:scale-105`}>
												...
												<div className="flex-1 min-w-0">
													<div className="flex flex-wrap items-center gap-2 mb-1">
														...
														{/* Type Badge */}
														<span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${typeCfg.badgeBg} ${typeCfg.color}`}>
															{typeCfg.label}
														</span>

														{/* Priority Badge */}
														{noti.priority !== "normal" && (
															<span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border flex items-center gap-1 ${priCfg.badge}`}>
																{priCfg.pulse && <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-ping inline-block" />}
																{priCfg.label}
															</span>
														)}
													</div>
													...
													<p className={`text-xs sm:text-sm leading-relaxed mt-1.5 ${
														noti.is_read ? "text-gray-400" : "text-gray-600 font-medium"
