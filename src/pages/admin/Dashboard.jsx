...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    ...
    const { data: events } = await supabase
      .schema("me_dataspace")
      .from("events")
      .select("eventID, title, fromDateTime, toDateTime, bannerURL")
      .gte("fromDateTime", today)
  ...
  return (
    <>
      <Header
        bgcolour="bg-gradient-to-r from-[#C1622A] to-[#E49E5F]"
        tcolour="text-white"
        logout="hidden"
        logo="hidden"
      />
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {quickActions.map(({ label, icon: Icon, path, action }) => (
            <button
              ...
              className="bg-white rounded-xl p-5 flex flex-col items-center gap-2 hover:shadow-md transition"
            >
