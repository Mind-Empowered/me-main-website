    ...
    try {
      const [
        ...
        supabase
          ...
          .from("events")
          .select(
            "eventID, title, fromDateTime, toDateTime, bannerURL, max_participants, max_volunteers, is_food_available, description, venue, venue_url, status",
          )
          .gte("fromDateTime", today)
              ...
              {upcomingEvents.map((event) => {
                ...
                return (
                  <div
                    key={event.eventID}
                    onClick={() =>
                      setExpandedEventId(isExpanded ? null : event.eventID)
                    }
                    className={`flex flex-col border border-gray-200 hover:border-[#C1622A]/50 rounded-xl p-4 bg-white shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer ${
                      isExpanded
                        ? "ring-1 ring-[#C1622A]/30 border-[#C1622A]/40"
                        : ""
                    }`}
                  >
                    ...
                    >
                      ...
                      <div className="space-y-3.5 text-xs text-gray-600">
                        ...
                        <div className="flex flex-wrap gap-2 items-center">
                          {event.status && (
                            <span
                              className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ${statusClass}`}
                            >
                              {event.status}
                            </span>
                          )}
                        ...
                        <div className="flex items-start gap-2.5">
                          <FaClock
                            className="text-[#C1622A] mt-0.5 flex-shrink-0"
                            size={13}
                          />
                          <div>
                            ...
                            {event.toDateTime && (
                              <p className="text-gray-500 mt-0.5">
                                ...
                                {new Date(event.toDateTime).toLocaleString(
                                  "en-GB",
                                  {
                                    dateStyle: "medium",
                                    timeStyle: "short",
                                ...
                                <div key={i} className="mt-1.5">
                                  ...
                                  {event.venue_url?.split(" | ")[i] && (
                                    <a
                                      ...
                                      className="text-[#C1622A] hover:underline font-medium inline-block mt-0.5"
                                    >
                                      View on Google Maps →
                                    </a>
                                  )}
                        ...
                        <div className="grid grid-cols-2 gap-3 bg-gray-50 p-2.5 rounded-lg border border-gray-100">
                          <div>
                            <p className="font-semibold text-gray-500 text-[10px] uppercase">
                              Max Volunteers
                            </p>
                            <p className="font-bold text-gray-800 text-sm mt-0.5">
                              {event.max_volunteers || "No limit"}
                            </p>
                          </div>
                          <div>
                            <p className="font-semibold text-gray-500 text-[10px] uppercase">
