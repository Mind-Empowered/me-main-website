...
import { logActivity } from "../../services/activityLog"; // ✅ ADD THIS IMPORT
...
const Calendar = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await logActivity({
        action: 'UPLOAD_CALENDAR',
        description: `Uploaded calendar for ${MONTHS[monthNum - 1]} ${yearNum}`,
  ...
  const confirmDelete = async () => {
    ...
    const calendarToDelete = calendars.find(c => c.id === confirmDeleteId);
    ...
    if (!error) {
      ...
      await logActivity({
        ...
        description: `Deleted calendar: ${MONTHS[calendarToDelete?.cal_month - 1]} ${calendarToDelete?.cal_year}`,
        entity_type: 'calendar',
  ...
  const saveEdit = async (id) => {
    ...
    try {
      ...
      const oldCalendar = calendars.find(c => c.id === id);
      ...
      await logActivity({
        ...
        description: `Updated calendar from ${MONTHS[oldCalendar?.cal_month - 1]} ${oldCalendar?.cal_year} to ${MONTHS[monthNum - 1]} ${yearNum}`,
        entity_type: 'calendar',
        entity_id: id
      });
